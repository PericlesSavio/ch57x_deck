...
class Backend:
    ...
    def download_url(self) -> str | None:
        target = _RELEASE_TARGETS.get(platform.machine())
        if not target:
            return None
        return (
            f"https://github.com/kriomant/{TOOL_NAME}/releases/download/"
            f"{TOOL_VERSION}/{TOOL_NAME}-{target}.tar.gz"
        )

    def install_tool(self) -> str:
        """Baixa a release oficial e instala em ~/.local/share/ch57x_deck/bin.

        Levanta RuntimeError com mensagem amigável em caso de falha.
        """
